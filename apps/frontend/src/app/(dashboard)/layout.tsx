...
import { dashboardNavigationSections, type NavigationItem } from '@/lib/navigation'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const DASHBOARD_SIDEBAR_PREF_KEY = 'sms.dashboard.sidebar.collapsed'

function isDashboardNavItemActive(pathname: string, item: NavigationItem) {
      ...
      <div>
        <button
          ...
          className={cn(
            ...
            collapsed ? 'justify-center gap-0 px-2 py-2.5' : 'justify-start gap-3 px-3 py-2',
            isActive
        ...
        >
          ...
          <span
            className={cn(
              'flex-1 overflow-hidden whitespace-nowrap text-left transition-all duration-200 ease-out',
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        collapsed ? 'justify-center gap-0 px-2 py-2.5' : 'justify-start gap-3 px-3 py-2 text-left',
        isActive
          ? 'bg-primary-50 text-primary-700'
          : 'text-foreground hover:bg-accent',
      )}
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem(DASHBOARD_SIDEBAR_PREF_KEY)
    if (stored != null) {
  ...
  useEffect(() => {
    window.localStorage.setItem(DASHBOARD_SIDEBAR_PREF_KEY, desktopSidebarCollapsed ? '1' : '0')
  ...
  const isDashboardItemVisible = useCallback(
    (item: NavigationItem) => {
      if (item.teacherOnly && !user?.teacherId) return false
      if (item.classTeacherOnly && user?.teacherId && !user?.classTeacherOfId) return false
      if (item.href === '/dashboard' && user?.teacherId) return false
      if (item.permission && !hasPermission(item.permission)) return false
      return true
    },
    [hasPermission, user?.teacherId, user?.classTeacherOfId],
  )

  const visibleSections = dashboardNavigationSections
    .map((section) => ({
      ...section,
      items: section.items.filter(isDashboardItemVisible),
    }))
        ...
        >
          ...
          >
            <Link
              ...
              title={sidebarCollapsed ? user?.schoolName || 'School' : undefined}
            >
          ...
          {/* Platform admin link */}
          <PlatformOnly>
            <div className={cn('border-b border-border py-2', sidebarCollapsed ? 'px-2' : 'px-3')}>
              <Link
                href="/dashboard/platform"
                title={sidebarCollapsed ? 'Platform Admin' : undefined}
                className={cn(
                  'flex items-center rounded-lg bg-primary-50 py-2 text-sm font-medium text-primary-700 transition-all duration-300 ease-out',
          ...
          <SchoolSwitcher collapsed={sidebarCollapsed} />
          ...
          <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border/50 bg-card/80 backdrop-blur-md px-4 lg:px-8">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-full border border-transparent pl-1.5 pr-4 py-1.5 outline-none transition-all duration-200 ease-out hover:border-border/30 hover:bg-black/5 active:scale-[0.98] dark:hover:bg-white/5 group">
                  <UserAvatar
                    src={user?.avatar}
